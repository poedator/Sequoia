...
sys.path.append("..")
from data_converter import convert_c4_dataset_eval, convert_cnn_dataset, convert_wiki_dataset  # noqa: E402
from Engine.Engine import GraphInferenceEngine, GraphInferenceEngineTG  # noqa: E402
from Engine.offload_engine import OffloadEngine  # noqa: E402
from Tree.SpecTree import SpecTree  # noqa: E402
from utils import _make_causal_mask, cuda_graph_for_residual, cuda_graph_for_sampling_without_replacement, get_sampling_logits  # noqa: E402
...
def simulation_fast(
    ...
    with torch.no_grad():
        for step, batch in tqdm(enumerate(dataloader), total=num_eval_steps):
            ...
            spectree = SpecTree(
                ...
                draft_model_engine=draft_model,
            ...
            t1 = time.time()
            while input_ids.shape[1] < 256 and terminate is False:
    ...
    print(
        "total time :{:.5f}s, latency :{:.5f}s, decoding step: {}, large model step: {}, {}".format(
            total_time, total_time / num_decoding_steps, num_decoding_steps, num_large_model_steps, num_decoding_steps / num_large_model_steps
        )
    )
...
def simulation_baseline(target_model: GraphInferenceEngineTG, dataloader: DataLoader, T=0.6, top_p=0.9, max_length=256):
    ...
    with torch.no_grad():
        for step, batch in tqdm(enumerate(dataloader), total=num_eval_steps):
            ...
            start_length = 0
            while inner_decoding_step < 32 and terminate is False:
                if inner_decoding_step == 0:
                    start_length = input_ids.shape[1]
                    logits = target_model.inference(
                        input_ids=input_ids,
                        storage_ids=storage_ids[:start_length],
                    ...
                    logits = target_model.inference(
                        input_ids=input_ids,
                        storage_ids=storage_ids[start_length + inner_decoding_step - 1 : start_length + inner_decoding_step],
                        position_ids=position_ids[..., start_length + inner_decoding_step - 1 : start_length + inner_decoding_step],
                        attn_mask=attn_mask[start_length + inner_decoding_step - 1 : start_length + inner_decoding_step, : start_length + inner_decoding_step][
                            None, None, :, :
                        ],
                    )[0][-1]
...
def simulation_benchmark(
    ...
    print("total time :{:.5f}s, latency :{:.5f}s, decoding steps: {}".format(total_time, total_time / num_decoding_steps, num_decoding_steps))
...
if args.Mode == "baseline":
    ...
    for i in range(draft_step - 1):
        ...
        sampling_callables[i] = cuda_graph_for_sampling_without_replacement(
            max_length=args.M, idx_len=idx_len, num_samples=num_samples, temperature=args.T, tree_size=tree_size
        )
    for i in range(draft_step - 1):
        ith_gather_list = []
